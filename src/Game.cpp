...
void Game::spawnSmallEnemies(std::shared_ptr<Entity> e)
{
    ...
    // create an angle from 0 to 2*pi (clockwise around unit circle) to determine angle of first small enemy leaving the large enemy
    float angle = asinf(vel.y / speed);
    if (vel.x < 0)  
    { 
        angle = M_PI - angle; // works for both signs of vel.y
    }
    else
    {
        angle += 2 * M_PI; // handles positive vel.x and negative vel.y, no effect on positive vel.x and vel.y (periodic handling internal)
    }
...
void Game::sMovement()
{
    ...
    // small enemies
    for (const std::shared_ptr<Entity>& smallEnemy : m_entityManager.getEntities("smallEnemy"))
    {
        CTransform& transform = smallEnemy->get<CTransform>();

        if (transform.pos.x <= m_enemyConfig.CR / 2 || m_window.getSize().x - transform.pos.x <= m_enemyConfig.CR / 2)
        {
            transform.velocity.x *= -1.0f;
        }
        if (transform.pos.y <= m_enemyConfig.CR / 2 || m_window.getSize().y - transform.pos.y <= m_enemyConfig.CR / 2)
        {
            transform.velocity.y *= -1.0f;
        }
...
{
    // using collision radius, not shape radius
    ...
    // player and small enemies
    for (auto& smallEnemy : map.at("smallEnemy"))
    {
        if (pPos.dist(smallEnemy->get<CTransform>().pos) <= m_playerConfig.CR + m_enemyConfig.CR / 2)
        {
            smallEnemy->destroy();
    ...
    for (auto& enemy : map.at("enemy"))
    {
        for (auto& bullet : map.at("bullet"))
        {
            if (enemy->get<CTransform>().pos.dist(bullet->get<CTransform>().pos) <= m_enemyConfig.CR + m_bulletConfig.CR)
            {
                ...
                m_score += enemy->get<CShape>().circle.getPointCount();
            }
        }
    }
    ...
    for (auto& smallEnemy : map.at("smallEnemy"))
    {
        for (auto& bullet : map.at("bullet"))
        {
            if (smallEnemy->get<CTransform>().pos.dist(bullet->get<CTransform>().pos) <= m_enemyConfig.CR + m_bulletConfig.CR)
            {
                smallEnemy->destroy();
                bullet->destroy();
...
void Game::sRender()
{
    ...
    // set the pos of the entity based on the entity's transform->pos
    // player
    ...
    for (auto& smallEnemy : m_entityManager.getEntities("smallEnemy"))
    {
        smallEnemy->get<CTransform>().rotAngle += 1.0f;
        smallEnemy->get<CShape>().circle.setRotation(smallEnemy->get<CTransform>().rotAngle);
    }
